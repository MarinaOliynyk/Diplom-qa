...
public class PaymentsCreditTest {
    ...
    @Test
    @DisplayName("Успешное отображение формы по нажатию на кнопку \"Купить в кредит\"")
    ...
    public void successfulPayOnCredit() {
        ...
        assertEquals("APPROVED", SqlHelperCredit.getCardStatusApproved());
    }

    @Test
    @DisplayName("Отклоненная банком покупка в кредит")
    public void invalidPayOnCredit() {
        ...
        assertEquals("DECLINED", SqlHelperCredit.getCardStatusDeclined());
    }

    @Test
    @DisplayName("Отклоненная банком покупка по несуществующей карте стороннего банка (в кредит)")
    ...
    void shouldCreateItem() {
        ...
        paymentsPage.proceedButton.click();
        paymentsPage.succeedNotification.should(appear, Duration.ofSeconds(15));
        assertEquals(SqlHelperCredit.getCardIdOrder(), SqlHelperCredit.getCardIdCredit());
    }
